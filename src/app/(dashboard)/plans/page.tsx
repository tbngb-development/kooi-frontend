...
import { PlanDetailDrawer } from "@/components/admin/PlanDetailDrawer";
import {
  CreditCard,
  Plus,
  Pencil,
            ...
            <table className="w-full border-collapse text-left text-sm">
              ...
              <tbody className="divide-y divide-surface-subtle font-medium text-text-primary">
                {plans.map((plan) => (
                  <tr
                    key={plan.id}
                    className="hover:bg-surface-muted/50 transition-colors cursor-pointer group"
                    onClick={() => setSelectedPlan(plan)}
                  >
                    <td className="px-5 py-4">
                      <p className="font-bold group-hover:text-error-600 transition-colors">
                        {plan.name}
                      </p>
                    ...
                    >
                      <div className="flex items-center gap-3">
                        <button
                          onClick={() => setSelectedPlan(plan)}
                          className="inline-flex items-center gap-1 text-xs font-semibold text-text-secondary hover:text-text-primary transition-colors cursor-pointer"
                        >
                          <Eye size={12} /> Inspect
                        </button>
                        <button
                          onClick={() => openEdit(plan)}
                          className="inline-flex items-center gap-1 text-xs font-semibold text-error-600 hover:text-error-500 transition-colors cursor-pointer"
                        >
                          <Pencil size={12} /> Edit
                        </button>
                      </div>
